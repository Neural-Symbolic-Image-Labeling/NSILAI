...
def get_total_list1(    ):
    f1=open("C:/Users/wyfwyf/AppData/Local/Programs/Python/Python37/purdue/campus.json",'r')
    f2=open("C:/Users/wyfwyf/AppData/Local/Programs/Python/Python37/purdue/city.json",'r')
    f3=open("C:/Users/wyfwyf/AppData/Local/Programs/Python/Python37/purdue/residential.json",'r')
    load_dict1 = json.load(f1)
    load_dict2 = json.load(f2)
    load_dict3 = json.load(f3)
    list=[load_dict1,load_dict2,load_dict3]
    total_object=[]
    total_list=[]
    image_number=1
...
'''def disjunctive(result_list):
    result=[]
    for i,rules in enumerate(result_list):
        n=''
        if i==0:
            n+='Rule'+str(i+1)+": "+rules
        elif i==len(result_list)-1:
            n+='or Rule'+str(i+1)+": "+rules
        else:
            n+='or Rule'+str(i+1)+": "+rules
        result.append(n) 
    return result'''
...
def main():
    ...
    #print(total_list)
    #print(threshold("campus(X)","num(2,5)",total_list))
    result_list=foil(target,total_list)
    #print(result_list)
    math_format=get_result_list(result_list)
    print(math_format)
    #print(NL(get_result_list(result_list)))
    natural_language=NL(math_format,target,result_list)
    print(natural_language)
    #end=time.time()
    #print(end-start)
    '''NL_result=disjunctive(natural_language)
    for i in NL_result:
        print(i)'''
    #print(rank_the_result(result_list))
    #print(rank_the_result(result_list))
